...
class UltimateAnonymize(object):
    ...
    def __choose_edge(self):
        ...
        pre_count = count_pre_security_index(self.__graph, partitions, self.__partitions_degree,
                                             self.__partitions_volume)
        ...
        for edge in self.__available_edges:
            ...
            security_index = count_security_index_by_pre(pre_count, edge, src_des, total_degree + 2,
                                                         self.__partitions_degree, self.__partitions_volume,
                                                         degree_distribute)
    ...
    def __anonymize(self):
        ...
        while count <= edge_sum:
            ...
            count += 1
        fin_partitions = self.__detection_func(self.__graph, **self.__func_args)
        for part in fin_partitions:
            logger.info(f'{part}')
        self.__graph.write_gml('test/after.gml')
